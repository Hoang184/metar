...
header = ('date,ceiling,wind,precip,freezing,phenomena' +'\n')
file_new = open("score.csv", mode="w")
...
while i < line_numbers:
    ...
    valid = cot1.iloc[i]
    try:
        score = bad_weather_classes(code)
        score = str(score).replace(')', '').replace('(', '')
        print(score)
    except:
        print("dong " + str(i) + " bi loi")
    file_new.write(valid + ',' + score + '\n')
    file_new.close
    i = i+1
